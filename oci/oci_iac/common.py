...
def repo_root() -> Path:
    """Return the repository root (two levels up from oci/oci_iac/)."""
    return Path(__file__).resolve().parent.parent.parent


def oci_dir() -> Path:
    return repo_root() / "oci"
...
class TransactionLog:
    """Structured JSON log mirroring the Bash json_step/json_finalize helpers."""

    def __init__(self, operation: str, log_dir: Optional[Path] = None):
        self.operation = operation
        d = log_dir or (oci_dir() / "local" / "logs")
        d.mkdir(parents=True, exist_ok=True)
        self.path = d / f"{operation}-{TIMESTAMP}.json"
